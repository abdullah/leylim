...
export default class Row {
  constructor(component) {
    ...
    Bus.subscribe(this.onUpdateRowTemplate);
  }
  ...
  toggleEditable(state) {
    const editableChild = this.templateNode.querySelectorAll(
      '[contenteditable]'
    );
   ...
   */
  onUpdateRowTemplate = () => {
    const data = dispatch('beforeRowUpdate', {
  ...
  attachEvents() {
    ...
    this.rowNode.ondragover = e => {
      e.preventDefault();
      const pos = this.rowNode.getBoundingClientRect();
      const { height } = pos;
      before = e.layerY < height / 2;
      this.rowNode.classList.add(before ? 'row-node-drag-up':'row-node-drag-down');
      this.rowNode.classList.remove(before ? 'row-node-drag-down':'row-node-drag-up');
    };

    this.rowNode.ondragleave = () => {
      this.rowNode.classList.remove('row-node-drag-down');
      this.rowNode.classList.remove('row-node-drag-up');
    };

    this.rowNode.ondrop = e => {
      const componentId = e.dataTransfer.getData('text/plain');
      Bus.fire(ADD_COMPONENT_WITH_DRAG_DROP, {
        rowId: this._uuid,
  ...
  render() {
    ...
    <a href="#" class="leylim-actions ${ACTIONS_HTML} ${this.rawHTMLmode
      ? ACTIONS_ACTIVE
