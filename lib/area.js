...
import { getIndex, createDom } from './utils';
...
export default class Area {
  ...
  */
   ...
   * @memberof Area
   */
  addComponent(options, drop) {
    if (!drop) {
      const newRow = dispatch('beforeRowAdd', options);
      this.rowList.push(new Row(newRow));
      this.render();
    } else {
      const { componentId, rowId, before } = options;
      const rowIndex = this.rowList.findIndex(value => value._uuid == rowId);
      const component = Leylim._components[componentId];
      const index = before ? rowIndex : rowIndex + 1;

      const newRow = dispatch('beforeRowAdd', component);
      this.rowList.splice(index, 0, new Row(newRow));
      this.render();
    }
    dispatch('rowAdded');
  }
  ...
  handleEvents = (type, params) => {
    ...
    switch (type) {
      case ADD_COMPONENT:
        ...
        break;
      case ADD_COMPONENT_WITH_DRAG_DROP:
        this.addComponent(params, true);
        break;
      case DUPLICATE_ROW:
  ...
  initArea() {
    ...
    const areaNode = createDom('DIV', null, { class: AREA_NODE });
  ...
  render() {
    ...
    if (this.rowList.length) {
      ...
      areaNode.innerHTML = `<div class="leylim__empty-area added"><span>😒</span> </br> No content</div>`;
    }
  }
}
